...
class _AdminScreenState extends State<AdminScreen> with SingleTickerProviderStateMixin {
                  ...
                  trailing: Row(
                    ...
                    children: [
                      ...
                      IconButton(
                        ...
                        onPressed: () => _showDeleteConfirmation(context, 'pilota', () {
                          // Qui implementare la chiamata API per eliminare il pilota
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('Pilota eliminato con successo')),
                          );
                          _refreshData();
                        }),
                      ),
                ...
                child: ListTile(
                  ...
                  trailing: Row(
                    ...
                    children: [
                      IconButton(
                        icon: const Icon(Icons.edit),
                        onPressed: () => _showConstructorForm(constructor: constructor),
                      ),
                      IconButton(
                        icon: const Icon(Icons.delete),
                        onPressed: () => _showDeleteConfirmation(context, 'team', () {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('Team eliminato con successo')),
                          );
                          _refreshData();
                        }),
                      ),
                ...
                child: ListTile(
                  ...
                  trailing: Row(
                    ...
                    children: [
                      ...
                      IconButton(
                        icon: const Icon(Icons.delete),
                        onPressed: () => _showDeleteConfirmation(context, 'notizia', () {
                          // Qui implementare la chiamata API per eliminare la notizia
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('Notizia eliminata con successo')),
                          );
                          _refreshData();
                        }),
                      ),
  ...
  void _showDeleteConfirmation(BuildContext context, String itemType, Function onConfirm) {
    showDialog(
      context: context,
  ...
  void _showDriverForm({Driver? driver}) {
    ...
    final TextEditingController teamController = TextEditingController(text: driver?.team ?? '');
    final TextEditingController pointsController = TextEditingController(text: driver?.points.toString() ?? '0');
    final TextEditingController imageUrlController = TextEditingController(text: driver?.imageUrl ?? '');
    final TextEditingController positionController = TextEditingController(text: driver?.position.toString() ?? '0');
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              // Qui implementare la chiamata API per salvare il pilota
              Navigator.pop(context);
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text(driver == null ? 'Pilota aggiunto con successo' : 'Pilota aggiornato con successo')),
              );
              _refreshData();
            },
  ...
  void _showConstructorForm({Constructor? constructor}) {
    final TextEditingController nameController = TextEditingController(text: constructor?.name ?? '');
    final TextEditingController pointsController = TextEditingController(text: constructor?.points.toString() ?? '0');
    final TextEditingController logoUrlController = TextEditingController(text: constructor?.logoUrl ?? '');
    final TextEditingController positionController = TextEditingController(text: constructor?.position.toString() ?? '0');

    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(constructor == null ? 'Aggiungi Team' : 'Modifica Team'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              ...
              Navigator.pop(context);
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text(constructor == null ? 'Team aggiunto con successo' : 'Team aggiornato con successo')),
              );
              _refreshData();
            },
            child: const Text('Salva'),
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              ...
              Navigator.pop(context);
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text(news == null ? 'Notizia aggiunta con successo' : 'Notizia aggiornata con successo')),
              );
              _refreshData();
            },
            child: const Text('Salva'),
