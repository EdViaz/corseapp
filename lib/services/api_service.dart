...
class ApiService {
  ...
  final String baseUrl = 'http://localhost/backend/api'; // Use 10.0.2.2 instead of localhost for Android emulator
  // Make sure your backend files are in the XAMPP htdocs directory (not in the project directory)
  ...
  Future<List<Constructor>> getConstructorStandings() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to load constructor standings: ${response.statusCode}');
      }
    } catch (e) {
      throw Exception('Network error: ${e.toString()}');
    }
